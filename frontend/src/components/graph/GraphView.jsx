...
export default function GraphView() {
  ...
  useEffect(() => {
    ...
    const allVisible = activeAct === 'all'
      ? graphData.nodes.map(n => ({ ...n }))
      : graphData.nodes
          .filter(n => n.act_counts[`act_${activeAct}`] > 0)
          .map(n => ({ ...n }))

    // Cap at MAX_NODES for performance
    const visibleNodes = allVisible.slice(0, MAX_NODES)
    ...
    if (visibleNodes.length === 0) {
      svg.append('text')
        .attr('x', width / 2).attr('y', height / 2 - 16)
        .attr('text-anchor', 'middle')
        .attr('fill', '#5C5470')
        .attr('font-size', '15px')
        .attr('font-family', 'Inter, sans-serif')
        .text('No characters appear in this act.')
      svg.append('text')
        ...
        .attr('font-size', '13px')
        .attr('font-family', 'Inter, sans-serif')
        .text('Try selecting a different act filter.')
      return
    }
    ...
    const nodes = nodeGroup.selectAll('g')
      ...
      .on('mouseleave', () => {
        ...
        edges.attr('stroke-opacity', 0.6).attr('stroke', '#5C5470')
      })
      .call(
        d3.drag()
          .on('start', (event, d) => {
  ...
  return (
    <div className="relative w-full h-full">
      <svg
        ...
        role="img"
        aria-label={`Character network graph with ${graphData?.nodes?.length || 0} characters`}
      />
